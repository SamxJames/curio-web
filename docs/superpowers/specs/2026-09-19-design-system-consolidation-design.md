...
| 19px | ArrivalHero teaser paragraph | **Open question for the plan**: TodayHero's equivalent teaser already uses a plain Tailwind size, not 19px — these two sibling screens are already inconsistent with each other. Plan should either (a) match ArrivalHero to whatever TodayHero uses, or (b) keep 19px as a deliberate "first-touch moment" exception and document why. Don't decide silently. |
