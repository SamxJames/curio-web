...
**Interfaces:**
- Produces: Tailwind utilities `text-micro`, `text-display`, `tracking-headline`, `tracking-label`, `tracking-eyebrow`, `tracking-eyebrow-wider`, `tracking-section`, `leading-display`, `leading-body`, `max-w-page`, `max-w-form` (all auto-generated by Tailwind v4 from the `@theme` namespaces `--text-*`/`--tracking-*`/`--leading-*`/`--container-*`), plus the plain CSS variable `--duration-theme` consumed directly in this same file's `body` rule. Every later task in Phase 1 and 2 consumes these.
- **Naming note (corrected after Task 1's review surfaced a real bug):** the tracking tokens are deliberately NOT named `tracking-wide`/`tracking-wider`/`tracking-widest` — those are Tailwind v4's own *built-in* tracking utilities (`0.025em`/`0.05em`/`0.1em`, see `node_modules/tailwindcss/theme.css`). Defining `@theme` keys with those exact names would silently override Tailwind's defaults app-wide, breaking the 7 files that already use plain `tracking-wide` today expecting Tailwind's real value (`app/account/page.tsx`, `app/login/page.tsx`, `app/play/page.tsx`, `components/HistoryList.tsx`, `components/PuzzleGame.tsx`, `components/StoryView.tsx`, `components/TodayHero.tsx`). `tracking-headline` and `tracking-label` don't collide with anything Tailwind ships and need no renaming.
...
@theme inline {
   ...
   * scale). Named tracking-eyebrow(-wider)/tracking-section rather than
   * Tailwind's own tracking-wide/wider/widest names — those are Tailwind
   * v4 built-ins (0.025em/0.05em/0.1em) already used elsewhere in this app
   * (e.g. StoryView.tsx, PuzzleGame.tsx); reusing those exact names here
   * would silently override Tailwind's defaults everywhere they're used,
   * not just add new ones. */
  --tracking-headline: -0.015em;
  --tracking-label: 0.06em;
  --tracking-eyebrow: 0.1em;
  --tracking-eyebrow-wider: 0.12em;
...
**Interfaces:**
- Consumes: `text-micro`, `text-display` (unused here), `tracking-label`/`tracking-eyebrow`/`tracking-eyebrow-wider`/`tracking-section`, `leading-display`/`leading-body` from Task 1.
...
| 68 | `text-[10.5px] tracking-[0.12em]` (subline) | `text-micro tracking-eyebrow-wider` |
| 79 | `text-[11px] tracking-[0.12em]` (Collection tab — a `<Link>`, not a `<button>`; see Task 10 Step 4) | `text-micro tracking-eyebrow-wider` |
| 89 | `text-[11px] tracking-[0.12em]` (History tab — also a `<Link>`) | `text-micro tracking-eyebrow-wider` |
| 99 | `text-[15px]` (empty-state message) | `text-base` |
| 163 | `text-[18px]` (h2) | `text-lg` |
| 164 | `text-[9.5px] tracking-[0.1em]` (tap to filter) | `text-micro tracking-eyebrow` |
...
| 230 | `text-[10px] tracking-[0.1em]` (clear button) | `text-micro tracking-eyebrow` |
| 241 | `text-[9.5px] tracking-[0.18em]` (month header, collection) | `text-micro tracking-section` |
...
| 312 | `text-[9.5px] tracking-[0.13em]` (lineage container — non-interactive wrapper; the per-language `<button>`s inside it at line 324 have no typography classes of their own, they inherit this) | `text-micro tracking-eyebrow-wider` |
...
| 354 | `text-[9.5px] tracking-[0.18em]` (month header, history) | `text-micro tracking-section` |
...
| 367 | `text-[9.5px] tracking-[0.1em]` (History POS tag) | `text-micro tracking-eyebrow` |

- [ ] **Step 2: Spacing — convert to Tailwind's dynamic spacing scale**

**Important, and different from how this step was first drafted:** Tailwind v4 generates spacing utilities (`m*-N`, `p*-N`, `gap-N`, `w-N`, `h-N`, etc.) dynamically as `calc(var(--spacing) * N)`, where `--spacing: 0.25rem` (4px) is defined once in Tailwind's own theme (`node_modules/tailwindcss/theme.css:325`). This is NOT a fixed enum the way Tailwind v3 worked — any numeric `N`, including fractional steps like `6.5` or `1.25`, is a real, valid utility. That means **every arbitrary spacing value below has an EXACT match** (`N = px ÷ 4`), not just a nearest-neighbor approximation. Use the exact value in every row — there is no "pick the visually closer one" judgment call left in this step, except the one explicitly marked as a deliberate exception at the end.

| Line (approx) | Old | New (exact — `N = px ÷ 4`) |
|---|---|---|
| 62 | `pb-[34px]` | `pb-8.5` (34 ÷ 4 = 8.5) |
| 63 | `pt-[30px]` | `pt-7.5` (30 ÷ 4 = 7.5) |
| 74 | `mt-[22px]` | `mt-5.5` (22 ÷ 4 = 5.5) |
| 79 | `mr-[24px]` | `mr-6` (24 ÷ 4 = 6, already exact) |
| 99 | `mt-[28px]` | `mt-7` (28 ÷ 4 = 7, already exact) |
| 161 | `pt-[28px]` | `pt-7` (already exact) |
| 169 | `mt-[14px]`, `h-[28px]`, `gap-[2px]` | `mt-3.5` (14÷4), `h-7` (28÷4), `gap-0.5` (2÷4) — all already exact |
| 194 | `gap-[6px]` | `gap-1.5` (6÷4, already exact) |
| 208 | `gap-[5px]`, `px-[9px]`, `py-[5px]` | `gap-1.25` (5÷4), `px-2.25` (9÷4), `py-1.25` (5÷4) — all exact |
| 219 | `mt-[14px]` | `mt-3.5` (exact) |
| 225 | `mt-[26px]` | `mt-6.5` (26÷4, exact) |
| 241 | `pt-[26px]` | `pt-6.5` (exact) |
| 259 | `mt-[30px]`, `pt-[22px]` | `mt-7.5` (30÷4), `pt-5.5` (22÷4) — both exact |
| 300 | `gap-[7px]` | `gap-1.75` (7÷4, exact) |
| 304 | `mt-[5px]` | `mt-1.25` (exact) |
| 312 | `gap-x-[7px]` | `gap-x-1.75` (exact) |
| 350 | `pt-[22px]` | `pt-5.5` (exact) |
| 354 | `pt-[26px]` | `pt-6.5` (exact) |
| 363 | `w-[42px]` | **Deliberate exception, not exact:** the exact match would be `w-10.5` (42÷4), but use `w-11` (44px) instead — a genuine 2px increase, chosen because it lands exactly on the 44px accessibility touch-target minimum for this date column, same motivation as `--text-micro`'s 11px floor elsewhere in this plan. Check this doesn't clip the widest date string ("Sep 18") before committing. |

Since every row except the last resolves to an *exact* pixel match, this step should produce **zero visible spacing change** in `CollectionScreen.tsx` — the only visible difference from this whole task is the `w-[42px]`→`w-11` widening (2px) plus the already-documented type-size/tracking/radius exceptions from Steps 1 and 3.
...
**Interfaces:**
- Consumes: `text-micro`, `text-display`, `tracking-eyebrow-wider` from Task 1.
...
```
(11px stays 11px — no visible change. 0.14em collapses into `--tracking-eyebrow-wider`'s 0.12em, a 0.02em nudge, within tolerance.)
...
**Note (caught during execution):** the doc comment above deliberately avoids writing a literal `app/**/opengraph-image.tsx` path inside the `/** ... */` block — that exact substring contains `**/`, which prematurely closes a JSDoc block comment regardless of context, producing a real syntax error. Spelled out as a parenthetical list of concrete paths instead.
...
Line 32 — replace `<p className="font-sans text-micro tracking-eyebrow-wider text-accent uppercase">` (from Task 3) with `<Eyebrow as="p" className="tracking-eyebrow-wider text-accent">`.
...
- Line 68 (subline) — `<Eyebrow className="mt-2.5 tracking-eyebrow-wider">`
- Line 164 ("tap to filter") — `<Eyebrow className="tracking-eyebrow">`
- Line 241 (month header, collection) — `<Eyebrow as="p" className="pt-7 pb-1 tracking-section">`
- Line 312 (lineage tags container — a non-interactive wrapper; the per-language `<button>`s inside it at line 324 have no typography of their own, they inherit `Eyebrow`'s via CSS inheritance, which is why this can become `<Eyebrow>` even though it wraps interactive children) — `<Eyebrow className="mt-2.5 flex flex-wrap gap-x-1.5 gap-y-0 tracking-eyebrow-wider">`; preserve the existing flex-wrap layout and the children (the `.map()` over `word.lineage`) exactly, only the wrapping element changes.
- Line 354 (month header, history) — same pattern as line 241
- Line 367 (History POS tag) — `<Eyebrow className="tracking-eyebrow">`
...
with:
```tsx
<Button variant="link" className="!text-accent !no-underline text-micro tracking-eyebrow uppercase" onClick={() => onToggleLanguage(activeLanguage!)}>
  clear
</Button>
```
...
- **Placeholder scan:** Task 13 (apply approved contrast fixes) is the one task in this plan that cannot carry exact final code, because its content is gated on Task 12's live approval — this is not a plan-writing gap, it's the correct shape for a task whose input doesn't exist yet. Every other task has real, complete code — including `CollectionScreen`'s spacing table (Task 2 Step 2), which was originally drafted as a set of "nearest neighbor" judgment calls before Tailwind v4's dynamic spacing scale (`calc(var(--spacing) * N)`, any fractional `N` valid) was confirmed during execution, at which point every value except one (`w-[42px]`→`w-11`, a deliberate 2px accessibility-floor widening) turned out to have an exact match and the table was corrected accordingly — no fabricated precision remains there. The one genuine open-ended judgment call left in the plan is `CollectionScreen`'s lineage-tag `ToggleGroup` data-lookup shape (Task 10 Step 3), which is flagged inline with clear bounds, not hidden.
